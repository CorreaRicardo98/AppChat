...
class InicioViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tablaChats.register(nib, forCellReuseIdentifier: "customCell")
...
extension InicioViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let celda = tablaChats.dequeueReusableCell(withIdentifier: "customCell", for: indexPath) as! TableViewCell
        
        if let index = chats[indexPath.row].remitente.firstIndex(of:"@"){
            let nombre = chats[indexPath.row].remitente.prefix(upTo: index)
            celda.nombrePersona.text = String(nombre)
        }
        celda.textoMensaje.text = chats[indexPath.row].cuerpoMsj
